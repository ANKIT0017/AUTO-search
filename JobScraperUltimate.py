...
def getFileName(extraText):
    ...
    file_name = (
        "jobs_"
        + calendar.month_name[timeObj.tm_mon]
        + "_"
        + str(timeObj.tm_mday)
        + "_"
        + time_of_day
...
def load_existing_jobs(file_name):
    if os.path.exists(file_name):
        ...
        existing_jobs = pd.read_csv(
            file_name, quoting=csv.QUOTE_NONNUMERIC, escapechar="\\", comment="#"
        )
        if "job_url" in existing_jobs.columns:
            return existing_jobs["job_url"].tolist()
        else:
            print(
                f"Warning: 'job_url' column not found in {file_name}. Deduplication skipped."
            )
            return []
    return []
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                total_jobs = (
                    total_jobs
                    if total_jobs < results_fetch_count
                    else results_fetch_count
                )
                print(
                    f"Fetched {len(jobs)} jobs (offset: {offset}), total so far from SkillSire: {len(all_jobs)} of {total_jobs}."
                )
...
try:
    ...
    matching_jobs = current_jobs[
        current_jobs["title_lower"].apply(
    ...
    if job_count > 0:
        ...
        with open(
            os.path.join(base_dir, "new_jobs_temp.json"), "w", encoding="utf-8"
        ) as f:
